...
def edge_dropout(graph, p=0.1):
    """Drop a fraction of edges uniformly at random. Training-time only."""
    if p <= 0 or graph.edge_index.shape[1] == 0:
        return graph
    new = graph.clone() if hasattr(graph, 'clone') else deepcopy(graph)
    keep = torch.rand(graph.edge_index.shape[1], device=graph.edge_index.device) > p
    new.edge_index = graph.edge_index[:, keep]
    if hasattr(graph, 'edge_attr') and graph.edge_attr is not None:
        new.edge_attr = graph.edge_attr[keep]
    return new


def build_warmup_cosine(optimizer, epochs: int, warmup: int = 20):
    """Linear warmup (0.1x lr → lr) for `warmup` epochs, then cosine decay."""
...
def train_one_fold(
    ...
    *,
    ...
    if warmup_epochs and warmup_epochs > 0:
        scheduler = build_warmup_cosine(optimizer, epochs, warmup=warmup_epochs)
    else:
    ...
    ce_loss = nn.CrossEntropyLoss(weight=class_w, label_smoothing=label_smoothing)
    mse_loss = nn.MSELoss()
    ...
    for epoch in range(1, epochs + 1):
        ...
        for sample in train_data:
            ...
            if edge_drop_p > 0:
                art = edge_dropout(art, edge_drop_p)
                ven = edge_dropout(ven, edge_drop_p)

            if use_airway:
                aw = sample["airway"].to(device)
                if edge_drop_p > 0:
                    aw = edge_dropout(aw, edge_drop_p)
...
def main():
    ...
    p.add_argument("--edge_drop_p", type=float, default=0.0,
                   help="Per-edge dropout probability during training (0 disables).")
    ...
    p.add_argument("--cache_format", choices=["auto", "unified", "tri"], default="auto",
                   help="Cache schema: auto (detect), unified (*.pkl), tri (*_tri.pkl).")
    args = p.parse_args()
    ...
    log.info("Regularisation: edge_drop_p=%.3f  label_smoothing=%.3f  warmup_epochs=%d  cache_format=%s",
             args.edge_drop_p, args.label_smoothing, args.warmup_epochs, args.cache_format)
    ...
    if args.cache_format == "tri":
        cache_path = Path(args.cache_dir)
        missing_tri = [cid for cid in labels if not (cache_path / f"{cid}_tri.pkl").exists()]
        if missing_tri:
            log.error("cache_format=tri but %d cases missing _tri.pkl (first=%s).",
                      len(missing_tri), missing_tri[0])
            sys.exit(1)
    elif args.cache_format == "unified":
        cache_path = Path(args.cache_dir)
        missing_u = [cid for cid in labels if not (cache_path / f"{cid}.pkl").exists()]
        if missing_u:
            log.error("cache_format=unified but %d cases missing .pkl (first=%s).",
                      len(missing_u), missing_u[0])
            sys.exit(1)
    if len(dataset) < 10:
