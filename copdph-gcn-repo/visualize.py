...
TOP10_COL_SPECS = [
    ("vein_bv5", ["静脉BV5"]),
    ("vein_branches", ["静脉血管分支数量"]),
    ("bv5_ratio", ["bv5_ratio"]),
    ("av_ratio", ["artery_vein_vol_ratio"]),
    ("total_bv5", ["肺血管BV5"]),
    ("lung_std", ["左右肺密度标准差"]),
    ("vein_bv10", ["静脉BV10"]),
    ("total_branches", ["肺血管血管分支数量"]),
    ("vessel_tort", ["肺血管弯曲度"]),
]


def _resolve_top10(rad_df):
    """Return dict name → resolved column or None."""
    return {name: _first_col(rad_df, needles) for name, needles in TOP10_COL_SPECS}
...
def build_enhanced_dataset(cache_dir, labels, rad_df, feat_cols):
    ...
    for case_id, label in labels.items():
        ...
        g_aug = augment_graph(
            g,
            commercial_total_vol_ml=_val(row, col_total),
            commercial_fractal_dim=_val(row, col_fractal),
            commercial_artery_density=_val(row, col_art_dens),
            commercial_vein_density=_val(row, col_vein_dens),
            pipeline_total_vol_ml=float(pipeline_vol) if pipeline_vol else None,
            commercial_vein_bv5=_val(row, top10["vein_bv5"]),
            commercial_vein_branch_count=_val(row, top10["vein_branches"]),
            commercial_bv5_ratio=_val(row, top10["bv5_ratio"]),
            commercial_artery_vein_vol_ratio=_val(row, top10["av_ratio"]),
            commercial_total_bv5=_val(row, top10["total_bv5"]),
            commercial_lung_density_std=_val(row, top10["lung_std"]),
            commercial_vein_bv10=_val(row, top10["vein_bv10"]),
            commercial_total_branch_count=_val(row, top10["total_branches"]),
            commercial_vessel_tortuosity=_val(row, top10["vessel_tort"]),
        )
        entry["graph"] = g_aug
        entry["row"] = row
...
def _rank_features_by_pvalue(df: pd.DataFrame, label_col: str = "_label") -> list[tuple[str, float]]:
    """Return numeric feature columns sorted by Mann-Whitney p (ascending)."""
    from scipy import stats
    ranked: list[tuple[str, float]] = []
    for col in df.columns:
        if col in ("patient_id", label_col) or not pd.api.types.is_numeric_dtype(df[col]):
            continue
        g0 = df.loc[df[label_col] == 0, col].dropna().values
        g1 = df.loc[df[label_col] == 1, col].dropna().values
        if g0.size < 3 or g1.size < 3:
            continue
        if np.nanstd(np.r_[g0, g1]) < 1e-12:
            continue
        try:
            _, p = stats.mannwhitneyu(g0, g1, alternative="two-sided")
        except ValueError:
            continue
        ranked.append((col, float(p)))
    ranked.sort(key=lambda x: x[1])
    return ranked


def plot_group_stats(rad_df, labels, out_path, cols=None, top_k: int = 8):
    """Boxplot the top-K most discriminative CT features (by Mann-Whitney p)."""
    ...
    ranked = _rank_features_by_pvalue(df)
    top = ranked[:top_k]
    if not top:
        logger.warning("no rankable features — skipping group stats plot")
        return
    ...
    for ax, (col, p) in zip(axes, top):
        g0 = df.loc[df["_label"] == 0, col].dropna().values
        g1 = df.loc[df["_label"] == 1, col].dropna().values
        ...
        for i, grp in enumerate([g0, g1], start=1):
            ...
            ax.scatter(np.full_like(grp, i, dtype=float) + jitter, grp, s=10,
                       alpha=0.55, color=["#0A3D91", "#8B1A1D"][i - 1])
        ax.set_title(f"{col}\nMann-Whitney p={p:.2e}", fontsize=9)
        ax.grid(True, axis="y", linestyle=":", alpha=0.4)
    for ax in axes[len(top):]:
        ax.axis("off")
    fig.suptitle(
        f"PH vs non-PH — Top {len(top)} commercial vessel features (n={len(df)})",
        fontsize=13, fontweight="bold",
    )
    plt.tight_layout()
    plt.savefig(out_path, dpi=180, bbox_inches="tight")
    plt.close(fig)
    logger.info("wrote %s (top-%d features)", out_path, len(top))
